...
meta %>% pull(2) %>% unique() %>% head(n=4) %>% get_id_of_surname(.,nk_num=2) %>% write(file='somatic_hn4.txt')
meta %>% pull(2) %>% unique() %>% head(n=4) %>% get_id_of_surname(.,nk_num=3) %>% write(file='germ_hn4.txt')

f4 <- meta %>% pull(2) %>% unique() %>% head(n=4)
