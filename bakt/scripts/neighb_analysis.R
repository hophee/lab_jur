...
bakt_tsvs <-  dir('bakta_isolates', full.names = T) %>% list.files(pattern = '\\d+\\.tsv$', full.names = TRUE)
all_tabs_1_nd <- sapply(bakt_tsvs, function(x){read_anat_tab(x) %>% tab_analysis(n=1)})
all_tabs_3_nd <- sapply(bakt_tsvs, function(x){read_anat_tab(x) %>% tab_analysis(n=3)})

#don't work
right_go <- sapply(all_tabs_1_nd[1,], function(x){
  x@result %>% mutate(orf = colnames(x))}) %>% bind_rows()
...
pdf('p_plots3.pdf', width = 12, height = 10)
get_go_plots(1, 'GO, right genes, nd=1', all_tabs_3_nd)
get_go_plots(2, 'GO, left genes, nd=1', all_tabs_3_nd)
get_kegg_plots(3, 'KEGG, right genes, nd=1', all_tabs_3_nd)
get_kegg_plots(4, 'KEGG, left genes, nd=1', all_tabs_3_nd)
dev.off()


right_go <- all_tabs_1_nd[1,] %>% 
  map(~ .x@result) %>%
  imap(~ mutate(.x, file_name = .y)) %>%
  bind_rows() %>% filter(ONTOLOGY == 'BP')%>% mutate(file_name =   sub('.*/', '', file_name)) %>% 
  mutate(ratio = as.numeric(sub("/.*", "", GeneRatio)) / as.numeric(sub(".*/", "", GeneRatio)))
